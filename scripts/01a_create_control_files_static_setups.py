...
def write_xml_file(filepath, salinity, replicate, group_count=4):
    ...
    ET.SubElement(output, "output_dir").text = f"../data_and_results/data_raw/community/static/{salinity:.3f}/{replicate:02d}"
...
# === MAIN LOOP ===
setup_name = "community_static"

for salinity in [0.035, 0.070, 0.105, 0.140]:
    for n in range(1, 11):
        xml_filename = f"../xml_control_files/{setup_name}_{salinity:.3f}_{n:02d}.xml"
        os.makedirs(f"../data_raw/community/static/{salinity:.3f}/{n:02d}",
                    exist_ok=True)
        write_xml_file(xml_filename, salinity=salinity, replicate=n)
