...
def write_xml_file(filepath, salinity, version, pft):
    ...
    ET.SubElement(bg, "salinity").text = f"../data_and_results/input_files/salinity/{salinity}_{version}.csv"
    ...
    ET.SubElement(output, "output_dir").text = f"../data_and_results/data_raw/one_plant/dynamic/{salinity}_{version}/pft_{pft}"
...
# === MAIN LOOP ===
setup_name = "one_plant_dynamic"
os.makedirs("../xml_control_files", exist_ok=True)

for salinity in [35, 70, 105]:
    for version in ["V1", "V2"]:
        for pft in range(1, 5):
            xml_filename = f"../xml_control_files/{setup_name}_{salinity}_{version}_pft_{pft}.xml"
            os.makedirs(f"../data_raw/one_plant/dynamic/{salinity}_{version}/pft_{pft}",
                        exist_ok=True)
            write_xml_file(xml_filename, salinity=salinity, version=version, pft=pft)
